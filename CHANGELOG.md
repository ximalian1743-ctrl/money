...
## 2026-04-17 · 复合找零一次性记账

**场景：** 在超市拿 10000 日元纸币买菜 6360，找零 3×1000 纸币 + 640 硬币。过去只能拆成两条流水手动记录，现在一屏搞定。

**改动：** 在 `TransactionForm` 支出类型下加入「使用现金支付（含找零）」复选框。勾选后填写：
- 付款金额（例：10000）
- 找零·纸币（例：3000，回同一纸币账户，净额抵消）
- 找零·硬币（例：640，进硬币账户）
- 实际支出自动计算 = 付款 − 找零纸币 − 找零硬币

保存时原子地写入两条流水：主支出（6360 从现金纸币出账）+ 转账（640 从现金纸币→现金硬币），两侧账户余额一步到位。仅纸币找零时（无硬币）退化为单条支出。

**文件：** `frontend/src/components/TransactionForm.tsx` / `frontend/src/pages/ManualEntryPage.tsx` / `frontend/src/test/manual-entry.test.tsx`
