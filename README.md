...
> 📦 **升级历程：** 2026-04-16 完成 4 轮产品化升级（40 项改动），详见 [`CHANGELOG.md`](CHANGELOG.md)。

## 核心功能

- **总览** — 净资产 + 人民币 / 日元 / 欠款 统一财富卡；快捷记账按钮；账户按类型分组（🏦/💵/💳/📱/🚇）
- **记账** — 手动表单 / AI 文字 / AI 小票图片；AI 解析结果可编辑，支持一次输入多笔
- **流水** — 日期分组 + 收支方向指示 + 彩色类别 Badge + 搜索 + 筛选 + 行内编辑
- **统计** — 月度报表 + 分类占比 + 每日趋势柱状图 + 预算警告 + CSV 导出
- **设置** — 汇率自动同步；AI 模型自动加载；全数据导出 / 导入
